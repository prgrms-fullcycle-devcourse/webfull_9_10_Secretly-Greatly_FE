...
/**
 * PanicMode — 패닉 핫키(ESC 두 번)로 거래 화면을 가리는 위장 전체화면.
 *
 * - ESC 를 짧은 간격으로 두 번 누르면 VS Code '설정' 페이지처럼 보이는
 *   풀스크린 오버레이가 떠서 실제 금융 화면을 즉시 가린다.
 * - 패닉 화면이 떠 있을 때 ESC 를 다시 누르면 닫힌다.
...
/* ── 위장 설정 화면 ────────────────────────────────────── */
function SettingsDisguise({ onClose }: { onClose: () => void }) {
...
export function PanicMode() {
  ...
  if (!open) return null;
  return <SettingsDisguise onClose={() => setPanic(false)} />;
}
